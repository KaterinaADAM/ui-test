...
public class Forum_Forum_Category extends ForumBase {
	...
	public void test03_ExportImportForum(){
		...
		String[] array= waitForAndGetElement(By.linkText(catName)).getAttribute("href").split("/");
		idCate = array[array.length-1];
		...
		magCat.importForums2Category(fileName + ".zip",idCate);
		deleteFile(fileName + ".zip");
